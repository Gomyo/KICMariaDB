...
포워드 엔지니어링
    모델링 작업한 내용을 -> Diagram -> 수작업(sql을 만들어서) -> 바로 DB로 넣어줌
    모델링 작업한 내용을 -> Diagram -> 바로 DB로 넣어줌 (자동화 작업으로 sql 안만들어도 됨)
리버스 엔지니어링
    DB -> 논리적(X)물리적(O) -> Diagram
    * jdbc(java database bridge connectivity)
        java를 이용한 데이터베이스 연결 라이브러리
        * Oracle이 아닌, 각 데이터베이스 회사에서 제공
